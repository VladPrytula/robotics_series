...
def cmd_train(args: argparse.Namespace) -> int:
    ...
    ent_coef_str = "auto" if args.ent_coef is None else f"{args.ent_coef}"
    target_ent_str = "auto" if args.target_entropy is None else f"{args.target_entropy}"
    ...
    print(f"[ch04] ent_coef={ent_coef_str}, target_entropy={target_ent_str}, gamma={args.gamma}")
    ...
    # Create environment with is_success tracking
    env = make_vec_env(env_id, n_envs=n_envs, seed=seed, monitor_kwargs={"info_keywords": ("is_success",)})

    # Infer episode length; Fetch tasks use 50
    max_ep_len = 50
    try:
        if hasattr(env, "envs") and env.envs and getattr(env.envs[0], "spec", None):
            max_ep_len = int(env.envs[0].spec.max_episode_steps or 50)
    except Exception:
        pass
    ...
    try:
        ...
        if use_her:
            ...
            # Explicit HER configuration for robustness
            # Note: online_sampling removed - not valid in SB3 2.x
            replay_buffer_kwargs = {
                "n_sampled_goal": n_sampled_goal,
                "goal_selection_strategy": goal_strategy,
            }

        # Determine entropy coefficient
        # "auto" uses target_entropy for auto-tuning (can collapse too fast on sparse Push)
        # Fixed value (e.g., 0.1) bypasses auto-tuning entirely
        ent_coef_val = "auto" if args.ent_coef is None else args.ent_coef
        target_ent = "auto" if args.target_entropy is None else args.target_entropy
    ...
    metadata = {
        ...
        "hyperparams": {
            ...
            "buffer_size": args.buffer_size,
            "learning_starts": args.learning_starts,
            "ent_coef": "auto" if args.ent_coef is None else args.ent_coef,
            "target_entropy": "auto" if args.target_entropy is None else args.target_entropy,
            "gamma": args.gamma,
            "learning_rate": 3e-4,
        },
...
def main() -> int:
    ...
    p_train.add_argument("--ent-coef", type=float, default=None,
                         help="SAC entropy coefficient (None -> auto; try 0.1 for fixed on Push)")
    p_train.add_argument("--target-entropy", type=float, default=None,
                         help="SAC target entropy (None -> auto=-dim(A)=-4; try -2.0 for Push)")
    p_train.add_argument("--gamma", type=float, default=0.98,
                         help="Discount factor (0.98 good for 50-step Fetch episodes)")
